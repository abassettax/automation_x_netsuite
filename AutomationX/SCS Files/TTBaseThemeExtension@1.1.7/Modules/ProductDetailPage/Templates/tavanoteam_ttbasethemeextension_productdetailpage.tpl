	...
	{{#if isPriceEnabled}}
		...
		{{else}}
			...
			{{!--<!-- <span class="product-views-price-exact" itemprop="offers" itemscope itemtype="https://schema.org/Offer"> -->--}}
				<meta itemprop="priceCurrency" content="{{currencyCode}}"/>
                {{#ifEquals model.item.custitem126 "Sale"}}
                    <div class="item-views-price-historical">Normally: <span class="item-views-price-historical-number"> {{model.item.custitem20_formatted}} </span></div>
                {{/ifEquals}}
				{{!<!-- Single -->--}}
				{{#if priceFormatted}}
