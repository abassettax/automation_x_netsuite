...
function latedaybeforesave(type) {
  ...
  if (nlapiGetRecordId() && type != 'delete') {
    ...
    nlapiLogExecution('debug',  'docLocation' , docLocation);
    //TODO: testing for reset of int shipping label field for rfs sen fn
    if (docLocation == '69') {
      nlapiLogExecution('debug',  'check true' , 'attemtping to clear integrated shipping label');
      var intShipping = nlapiGetFieldValue('generateintegratedshipperlabel');
      nlapiLogExecution('debug',  'integrated label fld' , intShipping);
      if (intShipping == 'T' || intShipping == true) {
        nlapiSetFieldValue('generateintegratedshipperlabel', "F");
      }
      var intShipping2 = nlapiGetFieldValue('generateintegratedshipperlabel');
      nlapiLogExecution('debug',  'integrated label fld 2' , intShipping2);
    }
  }
