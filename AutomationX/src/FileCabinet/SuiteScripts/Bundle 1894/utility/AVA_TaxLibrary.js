...
define(['N/runtime', 'N/log', 'N/search', 'N/url', 'N/record', 'N/error', 'N/https', 'N/format', './AVA_Library'],
	function(runtime, log, search, url, record, error, https, format, ava_library){
		var exchangeRate = 1, AVA_SubCurrency, AVA_InitTaxCall, AVA_FieldChangeTaxCall;
		var AVA_AdjustmentLineCnt, AVA_TempDocCode, AVA_BillableTimeArr, AVA_DocNo, AVA_InitFlag = 0;
		var AVA_ErrorCode = 0, AVA_ShipCode, AVA_HandlingCode, responseLineTax, BillCostToCust, AvaTax, Transaction;
		var AVA_BarcodesFeature, AVA_ConnectorStartTime, AVA_ConnectorEndTime, AVA_LatencyTime = 0;
		var AVA_Def_Addr1, AVA_Def_Addr2, AVA_Def_City, AVA_Def_State, AVA_Def_Zip, AVA_Def_Country;
		...
		var AVA_ShipToLatitude, AVA_ShipToLongitude, AVA_BillToLatitude, AVA_BillToLongitude, AVA_EntityUseCodeList, AVA_EntityUseCodeMapped;
			...
			try{
				if(configCache.AVA_ServiceTypes != null && configCache.AVA_ServiceTypes.search('TaxSvc') != -1){
					...
					if(AVA_FoundVatCountry == 1){
						...
						var currencyCode = cForm.addField({
							id: 'custpage_ava_currenycode',
							label: 'Currency Code',
							type: ui.FieldType.TEXT
						});
						currencyCode.updateDisplayType({
							displayType: ui.FieldDisplayType.HIDDEN
						});
						/*var isoCode = search.lookupFields({
							type: search.Type.CURRENCY,
							id: loadConfig.getValue('basecurrency'),
							columns: 'symbol'
						});
						currencyCode.defaultValue = isoCode.symbol;*/
						...
						if(transactionRecord.type == 'salesorder'){
							if(context.type == context.UserEventType.VIEW){
								...
								if(runtime.isFeatureInEffect('PURCHASEORDERS') == true && AVA_Validation(purchaseOrderRef) == false){
									cForm.addButton({
		...
		function AVA_TransactionBeforeSubmit(context, transactionRecord, configCache, connectorStartTime){
			var e1;
			AVA_LineCount = 0;
				...
				try{
					if(context.type == context.UserEventType.CREATE || context.type == context.UserEventType.EDIT){
						...
						if(AVA_RequiredFields(transactionRecord, configCache) == 0){
							...
							if(AVA_ErrorCode != 0 && AVA_ErrorCode != 1 && AVA_ErrorCode != 5 && AVA_ErrorCode != 6 && AVA_ErrorCode != 9 && AVA_ErrorCode != 10 && AVA_ErrorCode != 11 && AVA_ErrorCode != 15 && AVA_ErrorCode != 17 && AVA_ErrorCode != 19 && AVA_ErrorCode != 30 && AVA_ErrorCode != 31){
								//0,1,5,6,9,10,11,15,17,19,30,31
						...
						if(AVA_ResultCode == 'Success'){
							...
							if(configCache.AVA_EnableLogEntries == '1' && transactionRecord.getValue('custpage_ava_context') != 'WEBSTORE' && transactionRecord.getValue('custpage_ava_context') != 'WEBAPPLICATION' && transactionRecord.getValue('custpage_ava_context') != 'PAYMENTPOSTBACK'){
								var avaDocType = AVA_RecordType(transactionRecord.type);
		...
		function AVA_TransactionAfterSubmit(context, transactionRecord, configCache, connectorStartTime, details){
			AVA_LineCount = 0;
		...
		function AVA_GetEntityUseCodes(executionContext, entityId, addressId, mode){
			...
			try{
				var searchRecord = search.create({
					...
					filters:
						[
							['custrecord_ava_customerid_new', 'anyof', entityId],
							'and',
							['custrecord_ava_addressid_new', 'is', addressId]
						],
					columns: ['custrecord_ava_entityusemap_new']
				});
				var searchResult = searchRecord.run();
				searchResult = searchResult.getRange({
					start: 0,
				...
				for(var i = 0; searchResult != null && i < searchResult.length; i++){
					if(mode == '1'){
						entityMap = searchResult[i].getValue('custrecord_ava_entityusemap_new');
					}
					else{
			...
			if(context.type == context.UserEventType.EDIT || context.type == context.UserEventType.VIEW){
				...
				for(var i = 0; searchResult != null && i < searchResult.length; i++){
					if(context.type == context.UserEventType.EDIT){
						...
						url1 += '&doctype=' + avaDocType +'&doccode=' + transactionRecord.id + '&rectype=' + transactionRecord.type + '&ns_transid=' + transactionRecord.getValue('tranid') + '&AVA_FoundVatCountry=' + AVA_FoundVatCountry;
		...
		function AVA_RequiredFields(transactionRecord, configCache){
			...
			if(AVA_LocationPOS == 0){
				if(transactionRecord.getField('taxitem') != null){
					...
					taxcode = (taxcode == '-Not Taxable-') ? ((configCache.AVA_DefaultTaxCode != null && configCache.AVA_DefaultTaxCode.lastIndexOf('+') != -1) ? configCache.AVA_DefaultTaxCode.substring(0, configCache.AVA_DefaultTaxCode.lastIndexOf('+')) : configCache.AVA_DefaultTaxCode) : taxcode;
			...
			// 11. Check if Middle Man VAT ID is entered
			...
			// 12. Check if AVA_RecordType value is not '0'
						...
						if(shipTaxCode != null && shipTaxCode.length > 0){
							if(shipTaxCode != '-Not Taxable-'){
								if(shipTaxCode.indexOf(':') != -1){
									shipTaxCode = shipTaxCode.substring(shipTaxCode.indexOf(':') + 1);
								}
						...
						if(handlingTaxCode != null && handlingTaxCode.length > 0){
							if(handlingTaxCode != '-Not Taxable-'){
								if(handlingTaxCode.indexOf(':') != -1){
									handlingTaxCode = handlingTaxCode.substring(handlingTaxCode.indexOf(':') + 1);
								}
			...
			if((transactionRecord.getValue('custpage_ava_lineloc') == false || transactionRecord.getValue('custpage_ava_lineloc') == 'F')){
					...
					if((transactionRecord.type == 'creditmemo' || transactionRecord.type == 'cashrefund' || transactionRecord.type == 'returnauthorization') && transactionRecord.getValue('createdfrom') != null && transactionRecord.getValue('createdfrom').length > 0 && configCache.AVA_UseInvoiceAddress == true){
						...
						for(var i = 0; AVA_CreatedFromNS_Lines != null && i < AVA_CreatedFromNS_Lines.length; i++){
							if(transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'item', AVA_NS_Lines[line][1]) == AVA_CreatedFromNS_Lines[i][2]){
								...
								if(transactionRecord.getValue('taxitem') == null){
									taxcode = (AVA_TaxcodeArray[line] == '-Not Taxable-') ? ((configCache.AVA_DefaultTaxCode != null && configCache.AVA_DefaultTaxCode.lastIndexOf('+') != -1) ? configCache.AVA_DefaultTaxCode.substring(0, configCache.AVA_DefaultTaxCode.lastIndexOf('+')) : configCache.AVA_DefaultTaxCode) : AVA_TaxcodeArray[line];
								}
				...
				if(transactionRecord.getValue('ismultishipto') == true){
					...
					for(var ship = 0; AVA_ShipGroupTaxcodes != null && ship < AVA_ShipGroupTaxcodes.length; ship++){
						...
						shipLineTaxcode = (AVA_ShipGroupTaxcodes[ship][2] == null || AVA_ShipGroupTaxcodes[ship][2] == '-Not Taxable-') ? ((configCache.AVA_DefaultTaxCode != null && configCache.AVA_DefaultTaxCode.lastIndexOf('+') != -1) ? configCache.AVA_DefaultTaxCode.substring(0, configCache.AVA_DefaultTaxCode.lastIndexOf('+')) : configCache.AVA_DefaultTaxCode) : AVA_ShipGroupTaxcodes[ship][2];
			...
			if(recordType == 1){
				if((transactionRecord.type == 'creditmemo' || transactionRecord.type == 'cashrefund' || transactionRecord.type == 'returnauthorization') && transactionRecord.getValue('createdfrom') != null && transactionRecord.getValue('createdfrom').length > 0 && configCache.AVA_UseInvoiceAddress == true){
					if(typeId == 1){
						...
						if(transactionRecord.getValue('custpage_ava_context') == 'WEBSTORE'){
							if(AVA_CreateFromRecord.hasSubrecord('shippingaddress') == true){
								shipAddr = AVA_CreateFromRecord.getSubrecord({
						...
						if(transactionRecord.getValue('custpage_ava_context') == 'WEBSTORE'){
							if(transactionRecord.hasSubrecord('billingaddress') == true){
								billAddr = transactionRecord.getSubrecord({
					...
					if(typeId == 1){
						...
						if(transactionRecord.getValue('custpage_ava_context') == 'WEBSTORE'){
							if(transactionRecord.hasSubrecord('shippingaddress') == true){
								shipAddr = transactionRecord.getSubrecord({
									fieldId: 'shippingaddress'
								});
							}
						}
														...
														if(transactionRecord.getValue('custpage_ava_context') == 'WEBAPPLICATION' || transactionRecord.getValue('custpage_ava_context') == 'PAYMENTPOSTBACK' || transactionRecord.getValue('source') == 'SCIS'){
															if(configCache.AVA_DisableLocationCode == false){
																if((transactionRecord.getValue('custpage_ava_lineloc') == true || transactionRecord.getValue('custpage_ava_lineloc') == 'T') && AVA_CreateFromRecord.getSublistValue('item', 'location', AVA_CreatedFromNS_Lines[i][1]) != null && AVA_CreateFromRecord.getSublistValue('item', 'location', AVA_CreatedFromNS_Lines[i][1]).length > 0){
																	...
																	AVA_MultiShipAddArray[arrIndex][1][1] = configCache.AVA_Def_Addr1;
																	AVA_MultiShipAddArray[arrIndex][1][2] = configCache.AVA_Def_Addr2;
																	AVA_MultiShipAddArray[arrIndex][1][3] = '';
																	AVA_MultiShipAddArray[arrIndex][1][4] = configCache.AVA_Def_City;
																	AVA_MultiShipAddArray[arrIndex][1][5] = configCache.AVA_Def_State;
																	AVA_MultiShipAddArray[arrIndex][1][6] = configCache.AVA_Def_Zip;
																	AVA_MultiShipAddArray[arrIndex][1][7] = configCache.AVA_Def_Country;
																}
															}
															else{
																AVA_MultiShipAddArray[arrIndex][0] = 'Default Address';
		...
		function AVA_CalculateTax(transactionRecord, configCache){
			...
			try{
				...
				if(configCache.AVA_EnableLogEntries == '1' && transactionRecord.getValue('custpage_ava_context') != 'WEBSTORE' && transactionRecord.getValue('custpage_ava_context') != 'WEBAPPLICATION' && transactionRecord.getValue('custpage_ava_context') != 'PAYMENTPOSTBACK' && err.code != 'GetTaxError' && err.code != 'InvalidAddress' && err.code != 'ValueRequiredError'){
					var avaDocType = AVA_RecordType(transactionRecord.type);
				...
				if(err.code == 'SSS_CONNECTION_TIME_OUT' || err.code == 'SSS_REQUEST_TIME_EXCEEDED'){ // Logic for Retry of GetTax call - CLOUDERP-5326
					...
					try{
						...
						if(configCache.AVA_EnableLogEntries == '1' && transactionRecord.getValue('custpage_ava_context') != 'WEBSTORE' && transactionRecord.getValue('custpage_ava_context') != 'WEBAPPLICATION' && transactionRecord.getValue('custpage_ava_context') != 'PAYMENTPOSTBACK' && err.code != 'GetTaxError' && err.code != 'InvalidAddress'){
							var avaDocType = AVA_RecordType(transactionRecord.type);
		...
		function AVA_GetTaxBody(transactionRecord, configCache, avaDocType, scisFlag){
			...
			switch(configCache.AVA_CustomerCode){
				...
				case '6':
					...
					if(transactionRecord.getValue('custpage_ava_taxcodestatus') == 0){
						Transaction.customerCode = (custCode[7] != null && custCode[7].length > 0) ? custCode[7].substring(0, 50) : '';
					}
					else{
						Transaction.customerCode = (custCode != null && custCode.length > 0) ? custCode.substring(0, 50) : '';
					}
			...
			if(transactionRecord.getValue('currencysymbol') != null && transactionRecord.getValue('currencysymbol').length > 0){
				...
				/*if(AVA_FoundVatCountry == 1 && transactionRecord.getValue('exchangerate') != null && transactionRecord.getValue('exchangerate').length > 0){
			...
			if(transactionRecord.getValue('custbody_ava_vatbusinessid') != null && transactionRecord.getValue('custbody_ava_vatbusinessid').length > 0){
		...
		function AVA_GetTaxLines(transactionRecord, configCache, avaDocType, multiplier){
			...
			if(configCache.AVA_EntityUseCode == true){
				var searchEntityUseCode = search.create({
					type:  'customrecord_avaentityusecodes',
					columns: 'custrecord_ava_entityid'
				});
				var entityUseCodeResult = searchEntityUseCode.run();
				AVA_EntityUseCodeList = entityUseCodeResult.getRange({
					start: 0,
					end: 1000
				});
				
				if(shipToEntityFlag == 0){
					if((transactionRecord.type == 'creditmemo' || transactionRecord.type == 'cashrefund' || transactionRecord.type == 'returnauthorization') && transactionRecord.getValue('createdfrom') != null && transactionRecord.getValue('createdfrom').length > 0 && configCache.AVA_UseInvoiceAddress == true){
							...
							if(transactionRecord.getValue('custpage_ava_taxcodestatus') == 0 && transactionRecord.getValue('custpage_ava_context') != 'WEBSTORE'){
								if((AVA_CreateFromRecord.getValue('shipaddresslist') != null && AVA_CreateFromRecord.getValue('shipaddresslist').length > 0) || (AVA_CreateFromRecord.getValue('shipaddress') != null && AVA_CreateFromRecord.getValue('shipaddress').length > 0)){
									if(AVA_CreateFromRecord.getValue('custbody_ava_shiptousecode') != null && AVA_CreateFromRecord.getValue('custbody_ava_shiptousecode').length > 0){
										...
										for(var i = 0; AVA_EntityUseCodeList != null && i < AVA_EntityUseCodeList.length; i++){
											if(AVA_CreateFromRecord.getValue('custbody_ava_shiptousecode') == AVA_EntityUseCodeList[i].id){
												entityusecode = AVA_EntityUseCodeList[i].getValue('custrecord_ava_entityid');
												break;
											}
										}
										
										entityMapHeader = (entityusecode != null && entityusecode.length > 0) ? entityusecode.substring(0, 25) : '';
									}
								}
								else if((AVA_CreateFromRecord.getValue('billaddresslist') != null && AVA_CreateFromRecord.getValue('billaddresslist').length > 0) || (AVA_CreateFromRecord.getValue('billaddress') != null && AVA_CreateFromRecord.getValue('billaddress').length > 0)){
									if(AVA_CreateFromRecord.getValue('custbody_ava_billtousecode') != null && AVA_CreateFromRecord.getValue('custbody_ava_billtousecode').length > 0){
										...
										for(var i = 0; AVA_EntityUseCodeList != null && i < AVA_EntityUseCodeList.length; i++){
											if(AVA_CreateFromRecord.getValue('custbody_ava_billtousecode') == AVA_EntityUseCodeList[i].id){
												entityusecode = AVA_EntityUseCodeList[i].getValue('custrecord_ava_entityid');
												break;
											}
										}
									...
									if((transactionRecord.getValue('shipaddresslist') != null && transactionRecord.getValue('shipaddresslist').length > 0) || (transactionRecord.getValue('shipaddress') != null && transactionRecord.getValue('shipaddress').length > 0)){
										if(transactionRecord.getValue('custbody_ava_shiptousecode') != null && transactionRecord.getValue('custbody_ava_shiptousecode').length > 0 && transactionRecord.getValue('custbody_ava_shiptousecode') != ' '){
											if(transactionRecord.getValue('custbody_ava_shiptousecodetext') != null && transactionRecord.getValue('custbody_ava_shiptousecodetext').length > 0){
												...
												for(var i = 0; AVA_EntityUseCodeList != null && i < AVA_EntityUseCodeList.length; i++){
													if(transactionRecord.getValue('custbody_ava_shiptousecode') == AVA_EntityUseCodeList[i].id){
														entityusecode = AVA_EntityUseCodeList[i].getValue('custrecord_ava_entityid');
														break;
													}
												}
										...
										if(transactionRecord.getValue('custbody_ava_billtousecode') != null && transactionRecord.getValue('custbody_ava_billtousecode').length > 0 && transactionRecord.getValue('custbody_ava_billtousecode') != ' '){
											if(ransactionRecord.getValue('custbody_ava_billtousecodetext') != null && transactionRecord.getValue('custbody_ava_billtousecodetext').length > 0){
												...
												for(var i = 0; AVA_EntityUseCodeList != null && i < AVA_EntityUseCodeList.length; i++){
													if(transactionRecord.getValue('custbody_ava_billtousecode') == AVA_EntityUseCodeList[i].id){
														entityusecode = AVA_EntityUseCodeList[i].getValue('custrecord_ava_entityid');
														break;
													}
												}
				...
				if(transactionRecord.getValue('custpage_ava_context') != 'USERINTERFACE' && transactionRecord.getValue('ismultishipto') == true && transactionRecord.getValue('custpage_ava_taxcodestatus') != 0){
					var k = 0;
					AVA_EntityUseCodeMapped = new Array();
					
					var column = new Array();
					column[column.length] = search.createColumn({
						name: 'custrecord_ava_addressid'
					});
					column[column.length] = search.createColumn({
						name: 'custrecord_ava_entityusemap'
					});
					column[column.length] = search.createColumn({
						name: 'internalid',
						sort: search.Sort.ASC
					...
					var searchRecord = search.create({
						...
						filters: ['custrecord_ava_customerid', 'anyof', transactionRecord.getValue('entity')],
						columns: column
					});
					var searchResult = searchRecord.run();
					searchResult = searchResult.getRange({
					...
					while(searchResult != null && searchResult.length > 0){
						for(var i = 0; i < searchResult.length; i++){
							AVA_EntityUseCodeMapped[k] = new Array();
							AVA_EntityUseCodeMapped[k][0] = searchResult[i].getValue('custrecord_ava_addressid');
							AVA_EntityUseCodeMapped[k][1] = searchResult[i].getText('custrecord_ava_entityusemap');
			...
			// CONNECT-33951: Usage limit issue resolved for existing customer
			var vatAddr = new Array();
			if(AVA_FoundVatCountry == 1){
				vatAddr = AVA_GetVatAddresses(transactionRecord);
			}
			...
			for(var line = 0; AVA_TaxLines != null && line < AVA_TaxLines.length; line++){
				...
				if(AVA_TaxLines[line] == 'T'){
					...
					if(AVA_LocationPOS == 1 || locPOS == 1){
						if((transactionRecord.getValue('custpage_ava_lineloc') == false || transactionRecord.getValue('custpage_ava_lineloc') == 'F')){
							if(AVA_HeaderLocation != null && AVA_HeaderLocation.length > 0){
								taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'headerlocation');
							}
							else{
								taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'default');
							}
						}
						else{
							if(transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'location', AVA_NS_Lines[line][1]) != null && transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'location', AVA_NS_Lines[line][1]).length > 0){
								if(AVA_LocationArray[line] != null && AVA_LocationArray[line][0] != null && AVA_LocationArray[line][1] != null && AVA_LocationArray[line][1][0] != null && AVA_LocationArray[line][1][0].length > 0){
									taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'linelocation', AVA_LocationArray[line][1]);
								}
								else{
									taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'default');
								}
						...
						if(AVA_TaxCodes[line] == 0){
							...
							if(configCache.AVA_DisableLocationCode == true){
								taxLine.addresses.ShipFrom = AVA_GetAddressDetails(configCache, 'default');
							}
							else{
								if((transactionRecord.getValue('custpage_ava_lineloc') == false || transactionRecord.getValue('custpage_ava_lineloc') == 'F')){
									if((transactionRecord.type == 'creditmemo' || transactionRecord.type == 'cashrefund' || transactionRecord.type == 'returnauthorization') && transactionRecord.getValue('createdfrom') != null && transactionRecord.getValue('createdfrom').length > 0 && configCache.AVA_UseInvoiceAddress == true){
										...
										if(flag == 0){
											if(AVA_HeaderLocation != null && AVA_HeaderLocation.length > 0){
												taxLine.addresses.ShipFrom = AVA_GetAddressDetails(configCache, 'headerlocation');
											}
											else{
												taxLine.addresses.ShipFrom = AVA_GetAddressDetails(configCache, 'default');
											}
											...
											if(transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'location', AVA_NS_Lines[line][1]) != null){
												...
												for(var j = 0; j < AVA_LocationArray.length; j++){
													if(AVA_LocationArray[j][0] != null && (transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'location', AVA_NS_Lines[line][1]) == AVA_LocationArray[j][0])){
														taxLine.addresses.ShipFrom = AVA_GetAddressDetails(configCache, 'linelocation', AVA_LocationArray[j][1]);
														locationExists = 1;
														break;
													}
												}
									...
									if(flag == 1){
										if(transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'location', AVA_NS_Lines[line][1]) != null && transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'location', AVA_NS_Lines[line][1]).length > 0){
											if(AVA_LocationArray[line] != null && AVA_LocationArray[line][0] != null && AVA_LocationArray[line][1] != null && AVA_LocationArray[line][1][0] != null && AVA_LocationArray[line][1][0].length > 0){
												taxLine.addresses.ShipFrom = AVA_GetAddressDetails(configCache, 'linelocation', AVA_LocationArray[line][1]);
											}
											else{
												taxLine.addresses.ShipFrom = AVA_GetAddressDetails(configCache, 'default');
											}
										...
										if(AVA_CreatedFromNS_Lines[i][0] == 'item' && AVA_CreateFromRecord.getValue('ismultishipto') == true){
											...
											if(AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_CreatedFromNS_Lines[i][1]) != null && AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_CreatedFromNS_Lines[i][1]).length > 0){
												...
												if(transactionRecord.getValue('custpage_ava_context') == 'WEBAPPLICATION' || transactionRecord.getValue('custpage_ava_context') == 'PAYMENTPOSTBACK' || transactionRecord.getValue('source') == 'SCIS'){
													if(configCache.AVA_DisableLocationCode == false){
														if((transactionRecord.getValue('custpage_ava_lineloc') == true || transactionRecord.getValue('custpage_ava_lineloc') == 'T') && AVA_CreateFromRecord.getSublistValue('item', 'location', AVA_CreatedFromNS_Lines[i][1]) != null && AVA_CreateFromRecord.getSublistValue('item', 'location', AVA_CreatedFromNS_Lines[i][1]).length > 0){
															taxLine.addresses.ShipTo = AVA_GetAddressDetails(configCache, 'linelocation', AVA_LocationArray[i][1]);
														}
														else if(AVA_CreateFromRecord.getValue('location') != null && AVA_CreateFromRecord.getValue('location').length > 0){
															if(AVA_HeaderLocation == null || AVA_HeaderLocation.length == 0){
																AVA_HeaderLocation = AVA_GetAddresses(transactionRecord, configCache, AVA_CreateFromRecord.getValue('location'), 2);
															...
															taxLine.addresses.ShipTo = AVA_GetAddressDetails('configCache, headerlocation');
														}
														else{
															taxLine.addresses.ShipTo = AVA_GetAddressDetails(configCache, 'default');
														}
													}
													else{
														taxLine.addresses.ShipTo = AVA_GetAddressDetails(configCache, 'default');
													...
													taxLine.addresses.ShipTo = AVA_GetAddressDetails(configCache, 'headershipto');
												}
											}
										}
										else{
								...
								if(AVA_NS_Lines[line][0] == 'item' && transactionRecord.getValue('ismultishipto') == true){
									...
									if(transactionRecord.getSublistValue('item', 'shipaddress', AVA_NS_Lines[line][1]) != null && transactionRecord.getSublistValue('item', 'shipaddress', AVA_NS_Lines[line][1]).length > 0){
										...
										if(transactionRecord.getValue('custpage_ava_context') == 'WEBAPPLICATION' || transactionRecord.getValue('custpage_ava_context') == 'PAYMENTPOSTBACK' || transactionRecord.getValue('source') == 'SCIS'){
											if(configCache.AVA_DisableLocationCode == false){
												if((transactionRecord.getValue('custpage_ava_lineloc') == true || transactionRecord.getValue('custpage_ava_lineloc') == 'T') && transactionRecord.getSublistValue('item', 'location', AVA_NS_Lines[line][1]) != null && transactionRecord.getSublistValue('item', 'location', AVA_NS_Lines[line][1]).length > 0){
													taxLine.addresses.ShipTo = AVA_GetAddressDetails(configCache, 'linelocation', AVA_LocationArray[line][1]);
												}
												else if(transactionRecord.getValue('location') != null && transactionRecord.getValue('location').length > 0){
													if(AVA_HeaderLocation == null || AVA_HeaderLocation.length == 0){
														AVA_HeaderLocation = AVA_GetAddresses(transactionRecord, configCache, transactionRecord.getValue('location'), 2);
							...
							if(AVA_FoundVatCountry == 1){
								if(vatAddr != null && vatAddr.length > 0){
									if(vatAddr[0] == true){
										taxLine.addresses.goodsPlaceOrServiceRendered = AVA_GetAddressDetails(configCache, 'goodsplaceorserviceaddress', vatAddr);
									}
									
									if(vatAddr[7] == true){
										taxLine.addresses.pointOfOrderOrigin = AVA_GetAddressDetails(configCache, 'pointoforderoriginaddress', vatAddr);
									}
									
									if(vatAddr[14] == true){
										taxLine.addresses.import = AVA_GetAddressDetails(configCache, 'importaddress', vatAddr);
									}
								}								
							}
						}
									...
									if(transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'item', AVA_NS_Lines[line][1]) == AVA_CreatedFromNS_Lines[i][2]){
										if(AVA_CreatedFromNS_Lines[i][0] == 'item' && AVA_CreateFromRecord.getValue('ismultishipto') == true){
											...
											if(AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_CreatedFromNS_Lines[i][1]) != null && AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_CreatedFromNS_Lines[i][1]).length > 0){
												...
												taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'headershipto');
											}
										}
										else{
											taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'headershipto');
									...
									if((transactionRecord.type == 'creditmemo' || transactionRecord.type == 'cashrefund' || transactionRecord.type == 'returnauthorization') && transactionRecord.getValue('createdfrom') != null && transactionRecord.getValue('createdfrom').length > 0 && configCache.AVA_UseInvoiceAddress == true){
										...
										if(flag == 0){
											...
											if(transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'location', AVA_NS_Lines[line][1]) != null){
												...
												for(var j = 0; j < AVA_LocationArray.length; j++){
													if(AVA_LocationArray[j][0] != null && (transactionRecord.getSublistValue(AVA_NS_Lines[line][0], 'location', AVA_NS_Lines[line][1]) == AVA_LocationArray[j][0])){
														taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'linelocation', AVA_LocationArray[j][1]);
														locationExists = 1;
														break;
													}
												}
					...
					if(configCache.AVA_TaxCodeMapping == true){
						...
						if(taxcode != null && taxcode != ''){
							...
							if(AVA_Taxable[line] == 'F'){
								if(AVA_LineType[line] == 'Discount' && configCache.AVA_EnableDiscount == true && transactionRecord.getValue('custpage_ava_formdiscountmapping') == 0 && (configCache.AVA_DiscountTaxCode != null && configCache.AVA_DiscountTaxCode.length > 0)){
										...
										if(transactionRecord.getValue('custpage_ava_taxcodestatus') == 0){
											if((AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_CreatedFromNS_Lines[i][1]) != null && AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_CreatedFromNS_Lines[i][1]).length > 0) || shipToEntityFlag == 1){
												if(AVA_CreateFromRecord.getSublistValue('item', 'custcol_ava_shiptousecode', AVA_CreatedFromNS_Lines[i][1]) != null && AVA_CreateFromRecord.getSublistValue('item', 'custcol_ava_shiptousecode', AVA_CreatedFromNS_Lines[i][1]).length > 0){
													for(var i = 0; AVA_EntityUseCodeList != null && i < AVA_EntityUseCodeList.length; i++){
														if(AVA_CreateFromRecord.getSublistValue('item', 'custcol_ava_shiptousecode', AVA_CreatedFromNS_Lines[i][1]) == AVA_EntityUseCodeList[i].id){
															entityusecode = AVA_EntityUseCodeList[i].getValue('custrecord_ava_entityid');
															break;
														}
													}
											...
											if(transactionRecord.getValue('custpage_ava_usecodeusuage') != null && (transactionRecord.getValue('custpage_ava_usecodeusuage') == true || transactionRecord.getValue('custpage_ava_usecodeusuage') == 'T')){
												...
												if(shipToEntityFlag == 0 && AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_CreatedFromNS_Lines[i][1]) != null && AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_CreatedFromNS_Lines[i][1]).length > 0){
													for(var i = 0; AVA_EntityUseCodeMapped != null && i < AVA_EntityUseCodeMapped.length; i++){
														if(AVA_CreateFromRecord.getSublistValue('item', 'shipaddress', AVA_NS_Lines[line][1]) == AVA_EntityUseCodeMapped[i][0]){
															entityusecode = AVA_EntityUseCodeMapped[i][1];
															break;
														}
													}
													...
													if(AVA_CreateFromRecord.getValue('billaddresslist') != null && AVA_CreateFromRecord.getValue('billaddresslist').length > 0){
														for(var i = 0; AVA_EntityUseCodeMapped != null && i < AVA_EntityUseCodeMapped.length; i++){
															if(AVA_CreateFromRecord.getValue('billaddresslist') == AVA_EntityUseCodeMapped[i][0]){
																entityusecode = AVA_EntityUseCodeMapped[i][1];
																break;
															}
														}
										...
										if(shipToEntityFlag == 0 && transactionRecord.getSublistValue('item', 'shipaddress', AVA_NS_Lines[line][1]) != null && transactionRecord.getSublistValue('item', 'shipaddress', AVA_NS_Lines[line][1]).length > 0){
											if(transactionRecord.getSublistValue('item', 'custcol_ava_shiptousecode', AVA_NS_Lines[line][1]) != null && transactionRecord.getSublistValue('item', 'custcol_ava_shiptousecode', AVA_NS_Lines[line][1]).length > 0 && transactionRecord.getSublistValue('item', 'custcol_ava_shiptousecode', AVA_NS_Lines[line][1]) != ' '){
												if(transactionRecord.getSublistValue('item', 'custcol_ava_shiptousecodetext', AVA_NS_Lines[line][1]) != null && transactionRecord.getSublistValue('item', 'custcol_ava_shiptousecodetext', AVA_NS_Lines[line][1]).length > 0){
													...
													for(var i = 0; AVA_EntityUseCodeList != null && i < AVA_EntityUseCodeList.length; i++){
														if(transactionRecord.getSublistValue('item', 'custcol_ava_shiptousecode', AVA_NS_Lines[line][1]) == AVA_EntityUseCodeList[i].id){
															entityusecode = AVA_EntityUseCodeList[i].getValue('custrecord_ava_entityid');
															break;
														}
													}
												...
												for(var i = 0; AVA_EntityUseCodeMapped != null && i < AVA_EntityUseCodeMapped.length; i++){
													if(transactionRecord.getSublistValue('item', 'shipaddress', AVA_NS_Lines[line][1]) == AVA_EntityUseCodeMapped[i][0]){
														entityusecode = AVA_EntityUseCodeMapped[i][1];
														break;
													}
												}
											...
											if((transactionRecord.getValue('billaddresslist') != null && transactionRecord.getValue('billaddresslist').length > 0) || (transactionRecord.getValue('billaddress') != null && transactionRecord.getValue('billaddress').length > 0)){
												if(transactionRecord.getValue('custbody_ava_billtousecode') != null && transactionRecord.getValue('custbody_ava_billtousecode').length > 0 && transactionRecord.getValue('custbody_ava_billtousecode') != ' '){
													...
													for(var i = 0; AVA_EntityUseCodeMapped != null && i < AVA_EntityUseCodeMapped.length; i++){
														if(transactionRecord.getValue('billaddresslist') == AVA_EntityUseCodeMapped[i][0]){
															entityusecode = AVA_EntityUseCodeMapped[i][1];
															break;
														}
													}
		...
		function AVA_GetAddressDetails(configCache, addressType, addr){
		...
		function AVA_MultiShipHandlingLines(transactionRecord, configCache, lineNo, shipGroupTaxcode, multiplier){
					...
					for(var i = 0; AVA_LocationArray != null && i < AVA_LocationArray.length; i++){
						if(AVA_LocationArray[i][0] == sourceAddr){
							if(AVA_LocationArray[i][1][0] != null && AVA_LocationArray[i][1][0].length > 0){
								taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'linelocation', AVA_LocationArray[i][1]);
							}
							else{
								taxLine.addresses.SingleLocation = AVA_GetAddressDetails(configCache, 'default');
							}
				...
				if(taxCodestatus == 0){
					if((transactionRecord.getValue('custpage_ava_lineloc') == false || transactionRecord.getValue('custpage_ava_lineloc') == 'F')){
						...
						for(var i = 0; AVA_LocationArray != null && i < AVA_LocationArray.length; i++){
							if(AVA_LocationArray[i][0] == sourceAddr){
								if(AVA_LocationArray[i][1][0] != null && AVA_LocationArray[i][1][0].length > 0){
									taxLine.addresses.ShipFrom = AVA_GetAddressDetails(configCache, 'linelocation', AVA_LocationArray[i][1]);
								}
								else{
									taxLine.addresses.ShipFrom = AVA_GetAddressDetails(configCache, 'default');
								}
			...
			if(shipGroupTaxcode[3] == 'FREIGHT'){
				...
				if(transactionRecord.getValue('custpage_ava_taxcodestatus') == 0){
					if(shipGroupTaxcode[2] == '-Not Taxable-'){
						taxLine.taxCode = 'NT';
					}
					else{
		...
		function AVA_GetShipAndHandling(transactionRecord, configCache, lineNo, itemCode, multiplier){
			...
			if(itemCode == 'FREIGHT'){
				...
				if(transactionRecord.getValue('custpage_ava_taxcodestatus') == 0){
					if(transactionRecord.getValue('custpage_ava_shiptaxcode') == '-Not Taxable-'){
						taxLine.taxCode = 'NT';
					}
					else{
						if(transactionRecord.getValue('custbody_avashippingcode') == null || (transactionRecord.getValue('custbody_avashippingcode') != null && transactionRecord.getValue('custbody_avashippingcode').length == 0)){
				...
				if(transactionRecord.getValue('custpage_ava_handlingtaxcode') == '-Not Taxable-'){
					taxLine.taxCode = 'NT';
				}
			}
		...
		function AVA_SetTaxFlagsOnServer(transactionRecord, configCache, getTaxResult, avaDocType){
			...
			var defTaxCodeId, defTaxCode = (configCache.AVA_DefaultTaxCode != null) ? configCache.AVA_DefaultTaxCode.substring(0, configCache.AVA_DefaultTaxCode.lastIndexOf('+')) : '';
			...
			defTaxCodeId = (configCache.AVA_DefaultTaxCode != null) ? configCache.AVA_DefaultTaxCode.substring(configCache.AVA_DefaultTaxCode.lastIndexOf('+') + 1, configCache.AVA_DefaultTaxCode.length) : '';
			...
			if(transactionRecord.getValue('taxitem') != null){
				...
				if(AVA_TotalTax != 0){
					...
					if(taxcode != defTaxCode && taxcode != defTaxCode + '-POD' && taxcode != defTaxCode + '-POS' && taxcode == '-Not Taxable-'){
				...
				for(var i = 0; AVA_TaxRequestLines != null && i < AVA_TaxRequestLines.length; i++){
					...
					if(responseLineTax[i] == 'T' && (taxcode != defTaxCode && taxcode != defTaxCode + '-POD' && taxcode != defTaxCode + '-POS' && taxcode == '-Not Taxable-')){
						if(defTaxCode != taxcode.substring(0, defTaxCode.length)){
							transactionRecord.setSublistValue({
								sublistId: AVA_TaxRequestLines[i][0],
								fieldId: 'taxcode',
				...
				if((transactionRecord.getValue('ismultishipto') == null || transactionRecord.getValue('ismultishipto') == false) && (AVA_ShipCode == 'T' || AVA_HandlingCode == 'T')){
					...
					if(transactionRecord.getValue('shippingtaxcode') != null && parseFloat(transactionRecord.getValue('shippingcost')) > 0){
						...
						if(responseLineTax[i] == 'T' && AVA_ShipCode == 'T' && i < responseLineTax.length && (taxcode != defTaxCode && taxcode != defTaxCode + '-POD' && taxcode != defTaxCode + '-POS' && taxcode == '-Not Taxable-')){
							if(defTaxCode != taxcode.substring(0, defTaxCode.length)){
								transactionRecord.setValue({
					...
					if(transactionRecord.getValue('handlingtaxcode') != null && parseFloat(transactionRecord.getValue('handlingcost')) > 0){
						...
						if(responseLineTax[i] == 'T' && AVA_HandlingCode == 'T' && i < responseLineTax.length && (taxcode != defTaxCode && taxcode != defTaxCode + '-POD' && taxcode != defTaxCode + '-POS' && taxcode == '-Not Taxable-')){
							if(defTaxCode != taxcode.substring(0, defTaxCode.length)){
								transactionRecord.setValue({
		...
		function AVA_CancelTax(transactionRecord, configCache, cancelType, details){
			AvaTax = ava_library.mainFunction('AVA_InitSignatureObject', configCache.AVA_ServiceUrl);
			Transaction = new AvaTax.transaction();
			var defCompanyCode = (configCache.AVA_DefCompanyCode != null && configCache.AVA_DefCompanyCode.length > 0) ? configCache.AVA_DefCompanyCode : runtime.accountId;
			...
			try{
				...
				if(cRecord.getValue('custpage_ava_configobj') != null && cRecord.getValue('custpage_ava_configobj').length > 0){
					...
					if(configCache.AVA_ServiceTypes != null && configCache.AVA_ServiceTypes.search('TaxSvc') != -1){
										...
										if(cRecord.getCurrentSublistValue('item', 'custcol_ava_shiptousecode') != null){
											var response = https.get({
												url: url.resolveScript({
													scriptId: 'customscript_ava_recordload_suitelet',
													deploymentId: 'customdeploy_ava_recordload',
													params: {'type': 'customrecord_avaentityusemapping_new', 'custid': cRecord.getValue('entity'), 'addid': cRecord.getCurrentSublistValue('item', 'shipaddress')},
													returnExternalUrl: webstoreFlag
												})
											});
						...
						if(cRecord.getLineCount('item') > 1 && (context.fieldId == 'billaddresslist' || context.fieldId == 'shipaddresslist')){
								...
								if(cRecord.getValue('shipaddresslist') != null && cRecord.getValue('shipaddresslist').length > 0 && cRecord.getValue('custbody_ava_shiptousecode') != null){
									var response = https.get({
										url: url.resolveScript({
											scriptId: 'customscript_ava_recordload_suitelet',
											deploymentId: 'customdeploy_ava_recordload',
											params: {'type': 'customrecord_avaentityusemapping_new', 'custid': cRecord.getValue('entity'), 'addid': cRecord.getValue('shipaddresslist')},
											returnExternalUrl: webstoreFlag
										})
									});
									var shipToUseCode = response.body;
									...
									if(cRecord.getValue('billaddresslist') != null && cRecord.getValue('billaddresslist').length > 0 && cRecord.getValue('custbody_ava_billtousecode') != null){
										var response = https.get({
											url: url.resolveScript({
												scriptId: 'customscript_ava_recordload_suitelet',
												deploymentId: 'customdeploy_ava_recordload',
												params: {'type': 'customrecord_avaentityusemapping_new', 'custid': cRecord.getValue('entity'), 'addid': cRecord.getValue('billaddresslist')},
												returnExternalUrl: webstoreFlag
											})
										});
		...
		function AVA_CalculateTaxOnDemand(cRecord, connectorStartTime){
			...
			try{
				...
				if(AVA_ResultCode == 'Success'){
					...
					log.debug({
						title: 'CONNECTORMETRICS',
						details: 'Doc No - ' + docCode + ', (ConnectorTime - ' + connectorTime + ', LatencyTime - ' + AVA_LatencyTime + ')'
					});
