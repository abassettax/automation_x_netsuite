/******************************************************************************************************
	Script Name - AVA_SUT_CommittedList.js
	...
	function(ui, search, url, runtime, file, ava_library){
		function onRequest(context){
			...
			if(checkServiceSecurity == 0){
				checkServiceSecurity = ava_library.mainFunction('AVA_CheckSecurity', 1);
			}
			
			if(checkServiceSecurity == 0){
				var form = ui.createForm({
					title: 'AvaTax Committed Transaction List'
				});
		...
		function AVA_GetTransactionData(){
			...
			var searchRecord = search.create({
				...
				filters: ['custrecord_ava_documentstatus', 'equalto', 3],
				columns:
					[
