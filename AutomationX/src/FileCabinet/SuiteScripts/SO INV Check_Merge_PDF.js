...
/////////////////////////////////////////////////////// save pfd of SO
 var SoFile = nlapiPrintRecord('transaction', recId,'PDF');
...
/////////////////////////////////////////////////////// end save FF
  ...
  /////////////////////////////////////////////////////// save pfd of walkin order
    if(nlapiLookupField("transaction" , recId, 'custbodysavedsignature') || nlapiLookupField("transaction" , recId, 'custbodycustbodysavedsignature_ff'))
    {
 var WISOFile = nlapiPrintRecord('transaction', recId,'PDF');
  //set target folder in file cabinet
...
*/////////////////////////////////////////////////////// end get echosign agreements Sales Order  
  ...
  var relatedfilters = new Array();
   ...
   relatedfilters[3] = new nlobjSearchFilter("internalid","createdfrom","anyof",recId);
...
nlapiLogExecution('debug','fileid', relatedfileNameTabid );
  ...
  var relatedfileNameTabSOid = relatedresultsSO.getValue("internalid","file","GROUP");
     ...
     if(filesName.indexOf(relatedfileNameTabSO) == -1 && relatedfileNameTabSOid)
         { filesa.push(relatedfileNameTabSOid);  filesName.push(relatedfileNameTabSO);}
        }
   ...
   var path = "https://system.na3.netsuite.com" + nlapiLoadFile(relatedfileName).getURL();
    ...
    //var contents = file.getValue();
     ...
     var file = nlapiLoadFile(relatedfileName)
             ...
             var path = "https://system.na3.netsuite.com" +nlapiLoadFile(relatedfileName).getURL(); //"https://system.na3.netsuite.com" +nlapiLoadFile(relatedfileName).getURL();  //file.getValue();// nlapiLoadFile(relatedfileName).getValue();  //      // nlapiLogExecution('debug','path', path );     //file.setEncoding('UTF-8');
            strName += nlapiEscapeXML(path);
...
//aaaaaa///////////////////////////////////////////////////// end get other related docs
 ...
 var CFrelateddoc =  nlapiLookupField('salesorder', recId, 'custbody75');
...
var strName = "<table >";
 strName += "<tr>";
     strName += "<td>";
            ...
            var path = "https://system.na3.netsuite.com" +nlapiLoadFile(CFrelateddoc).getURL();
            strName += nlapiEscapeXML(path);
            strName += "\"></img>";
     strName += "</td></tr>";
strName += "</table>";
...
//-------------------------------------------------------------------Get PDF from created from
 if( CFrelateddoctype == "PDF File" )//&& CFrelateddoc != "")
   {
 var pdfpath = "https://system.na3.netsuite.com" +nlapiLoadFile(CFrelateddoc).getURL();
 var pdf_fileURL = nlapiEscapeXML(pdfpath); 
 xml += "<pdf src='"+ pdf_fileURL +"'/>";
  }
