...
function TJINC_ATXNED_UpdateSubCustomers(s_type)
{
	if (!b_ATXNED_log){ b_ATXNED_log = (navigator.userAgent == s__ATXNED_agent); }
	if (b_ATXNED_log){ nlapiLogExecution('DEBUG', 'TJINC_ATXNED_UpdateSubCustomers', 'IN');}
	 
	if(nlapiGetFieldText('parent') == '' && s_type == 'edit'){

		var a_filters = [], a_columns = [], o_parentCustomer = null, o_activeItems = {},o_activeItems_b = {},o_activeItems_c = {};

		o_parentCustomer = nlapiLoadRecord(nlapiGetRecordType(), nlapiGetRecordId()); 
